...
import {getRegionsSearch, getInputTips} from "./api/data";
...
export default {
  ...
  methods: {
    ...
    inputTips(queryString) {
      ...
      //       // key: '06d1ff87930d3d2a297c619c40fd979c'
      //     },
      //   })
      //
      //
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          //     })
          //   )
          getRegionsSearch(this.regionsSearchForm.values)


            .then((response) => {
              this.results = response.data.complex
              this.notify()
            })
            .catch(() => {})
        } else {
